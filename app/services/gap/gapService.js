...
export default class GapService {
  ...
  // 2. Fetch all GAP certificates from API and store in DB (NEW APPROACH)
  // 3. Log attempt start/results and final results
  // 4. Return summary result object
  static async fetchAllGap(maxAttempts = GAP_CONFIG.DEFAULT_MAX_ATTEMPTS) {
    ...
    const insertResult = await bulkInsertOrUpdateGap(result.data);

    const countAfter = await this._getDatabaseCount();

    GapLogger.logFinalResults("ALL", countAfter, 1, maxAttempts, STATUS.SUCCESS);
